...
mpc_utils::Status RunValueProviderVectorOLE(T x, int y_len, absl::Span<T> span_output) {
	...
	int *indices_notobliv= (int *)calloc(mc_args.t, sizeof(int ));
...
mpc_utils::Status RunIndexProviderVectorOLE(absl::Span<const T> y, absl::Span<const int64_t> indices, absl::Span<T> span_output){
    ...
    uint8_t **beta_vector=(uint8_t **)calloc(mc_args.t, sizeof(uint8_t *) );
    ...
    if(mc_args.print_stdout)log_info("Indices provided by caller\n");
   	...
   	for (int i = 0; i < mc_args.t; ++i){
   		indices_notobliv[i]=(int)indices.at(i);
   		indices_notobliv_vec.push_back((int)indices.at(i));
   	}
    ...
    y_args->all_buckets_array=mc_args.all_buckets_array;
    y_args->set_beta=1;
    y_args->beta_vector=beta_vector; 
    y_args->cprg=mc_args.cprg;
